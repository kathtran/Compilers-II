...
class SC0Gen {
  ...
  static List<String> gen(Ast0.Assign n) throws Exception {
    ...
    //  ... NEED CODE ...
    if (!vars.contains(n.lhs.nm)) {
      vars.add(n.lhs.nm);
    }
    int idx = vars.indexOf(n.lhs.nm);
    code.add("STORE " + idx);
  ...
  static List<String> gen(Ast0.While n) throws Exception {
    ...
    //  ... NEED CODE ...
    code.add("IFZ +" + (scode.size() + 2));
    code.addAll(scode);
    code.add("GOTO -" + (code.size()));
